...
void main() {
  test('test roundup.xml', () async {
    final XmlDocument _docXML =
        XmlDocument.parse(await File('./test/roundup.xml').readAsString());
    ...
    expect(feed.podcastCoverUrl,
        'http://www.radionikkei.jp/archives/program/roundup/podcast.jpg');
    ...
    expect(feed.episodes.first.enclosure.url,
        'http://podcasting.radionikkei.jp/podcasting/roundup/roundup-201127.mp3');
  ...
  test('test hiromaeda.xml', () async {
    final XmlDocument _docXML =
        XmlDocument.parse(await File('./test/hiromaeda.xml').readAsString());
    final feed = Podcast.fromXml(_docXML);
